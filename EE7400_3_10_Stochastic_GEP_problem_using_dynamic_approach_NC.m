...
for w=1:4
    ...
    Constraints=[Constraints, P_existing_gen(1,1,w) - Pline(1,1,w) == 0];
    Constraints=[Constraints, P_can_gen(1,1,w) + Pline(1,1,w) == Demand(1,1,w)];
    Constraints=[Constraints, Pline(1,1,w) == 500*(Del1(1,1,w)-Del2(1,1,w))];
    Constraints=[Constraints, -500 <= Pline(1,1,w) <= 500];
    ...
    Constraints=[Constraints, -pi <= Del2(1,1,w) <= pi];
    Constraints=[Constraints, Del1(1,1,w) == 0];
    
    Constraints=[Constraints, 35-lamda1(1,1,w)+mu_ex_gen(1,1,w)>=0];
    Constraints=[Constraints, 25-lamda2(1,1,w)+mu_can_gen(1,1,w)>=0];
    
    Constraints=[Constraints, lamda1(1,1,w)-lamda2(1,1,w)-mu_line(1,1,w)+mu_line_max(1,1,w)-mu_line_min(1,1,w)==0];
    Constraints=[Constraints, -500*mu_line(1,1,w)+mu_del2_max(1,1,w)-mu_del2_min(1,1,w)==0];
    Constraints=[Constraints, 500*mu_line(1,1,w)+ mu_del_ref(1,1,w)==0];
    
    Constraints=[Constraints, mu_ex_gen(1,1,w)>=0];
    Constraints=[Constraints, mu_can_gen(1,1,w)>=0];
    
    Constraints=[Constraints, mu_line_max(1,1,w)>=0];
    Constraints=[Constraints, mu_line_min(1,1,w)>=0];
    Constraints=[Constraints, mu_del1_max(1,1,w)>=0];
    Constraints=[Constraints, mu_del1_min(1,1,w)>=0];
    ...
    Constraints=[Constraints, 35*P_existing_gen(1,1,w) + 25*P_can_gen(1,1,w)==Demand(1,1,w)*lamda2(1,1,w)-400*mu_ex_gen(1,1,w)-sum(z_cqo(:,1,w))-...
                              500*(mu_line_max(1,1,w)+mu_line_min(1,1,w)) - pi*(mu_del2_max(1,1,w)+mu_del2_min(1,1,w))];
    ...
    Constraints=[Constraints, P_existing_gen(1,2,w) - Pline(1,2,w) == 0];
    Constraints=[Constraints, P_can_gen(1,2,w) + Pline(1,2,w) == Demand(1,2,w)];
    Constraints=[Constraints, Pline(1,2,w) == 500*(Del1(1,2,w)-Del2(1,2,w))];
    Constraints=[Constraints, -500 <= Pline(1,2,w) <= 500];
    ...
    Constraints=[Constraints, -pi <= Del2(1,2,w) <= pi];
    Constraints=[Constraints, Del1(1,2,w) == 0];
    
    Constraints=[Constraints, 35-lamda1(1,2,w)+mu_ex_gen(1,2,w)>=0];
    Constraints=[Constraints, 25-lamda2(1,2,w)+mu_can_gen(1,2,w)>=0];

    Constraints=[Constraints, lamda1(1,2,w)-lamda2(1,2,w)-mu_line(1,2,w)+mu_line_max(1,2,w)-mu_line_min(1,2,w)==0];
    Constraints=[Constraints, -500*mu_line(1,2,w)+mu_del2_max(1,2,w)-mu_del2_min(1,2,w)==0];
    Constraints=[Constraints, 500*mu_line(1,2,w)+ mu_del_ref(1,2,w)==0];
    
    Constraints=[Constraints, mu_ex_gen(1,2,w)>=0];
    Constraints=[Constraints, mu_can_gen(1,2,w)>=0];
    
    Constraints=[Constraints, mu_line_max(1,2,w)>=0];
    Constraints=[Constraints, mu_line_min(1,2,w)>=0];
    Constraints=[Constraints, mu_del1_max(1,2,w)>=0];
    Constraints=[Constraints, mu_del1_min(1,2,w)>=0];
    ...
    Constraints=[Constraints, 35*P_existing_gen(1,2,w)+25*P_can_gen(1,2,w)==Demand(1,2,w)*lamda2(1,2,w)-400*mu_ex_gen(1,2,w)-sum(z_cqo(:,2,w))-...
                              500*(mu_line_max(1,2,w)+mu_line_min(1,2,w)) - pi*(mu_del2_max(1,2,w)+mu_del2_min(1,2,w))];
                          
    Constraints=[Constraints, z_cqo(:,2,w)== p_cq*mu_can_gen(1,2,w)-z_cqo_cap(:,2,w)];
    Constraints=[Constraints, 0<=z_cqo(:,2,w)<=u_cq(:,1,w)*M];
    Constraints=[Constraints, 0<=z_cqo_cap(:,2,w)<=(1-u_cq(:,1,w))*M];
    
    
    %% Time period 2, Operating condition 1
    Constraints=[Constraints, P_existing_gen(2,1,w) - Pline(2,1,w) == 0];
    Constraints=[Constraints, P_can_gen(2,1,w) + Pline(2,1,w) == Demand(2,1,w)];
    Constraints=[Constraints, Pline(2,1,w) == 500*(Del1(2,1,w)-Del2(2,1,w))];
    Constraints=[Constraints, -500 <= Pline(2,1,w) <= 500];
    ...
    Constraints=[Constraints, -pi <= Del2(2,1,w) <= pi];
    Constraints=[Constraints, Del1(2,1,w) == 0];
    
    Constraints=[Constraints, 35-lamda1(2,1,w)+mu_ex_gen(2,1,w)>=0];
    Constraints=[Constraints, 25-lamda2(2,1,w)+mu_can_gen(2,1,w)>=0];
    
    Constraints=[Constraints, lamda1(2,1,w)-lamda2(2,1,w)-mu_line(2,1,w)+mu_line_max(2,1,w)-mu_line_min(2,1,w)==0];
    Constraints=[Constraints, -500*mu_line(2,1,w)+mu_del2_max(2,1,w)-mu_del2_min(2,1,w)==0];
    Constraints=[Constraints, 500*mu_line(2,1,w)+ mu_del_ref(2,1,w)==0];
    
    Constraints=[Constraints, mu_ex_gen(2,1,w)>=0];
    Constraints=[Constraints, mu_can_gen(2,1,w)>=0];
    
    Constraints=[Constraints, mu_line_max(2,1,w)>=0];
    Constraints=[Constraints, mu_line_min(2,1,w)>=0];
    Constraints=[Constraints, mu_del1_max(2,1,w)>=0];
    Constraints=[Constraints, mu_del1_min(2,1,w)>=0];
    ...
    Constraints=[Constraints, 35*P_existing_gen(2,1,w) + 25*P_can_gen(2,1,w)==Demand(2,1,w)*lamda2(2,1,w)-400*mu_ex_gen(2,1,w)-sum(z_cqo(:,3,w))-...
                              500*(mu_line_max(2,1,w)+mu_line_min(2,1,w)) - pi*(mu_del2_max(2,1,w)+mu_del2_min(2,1,w))];
    ...
    Constraints=[Constraints, P_existing_gen(2,2,w) - Pline(2,2,w) == 0];
    Constraints=[Constraints, P_can_gen(2,2,w) + Pline(2,2,w) == Demand(2,2,w)];
    Constraints=[Constraints, Pline(2,1,w) == 500*(Del1(2,1,w)-Del2(2,2,w))];
    Constraints=[Constraints, -500 <= Pline(2,2,w) <= 500];
    ...
    Constraints=[Constraints, -pi <= Del2(2,2,w) <= pi];
    Constraints=[Constraints, Del1(2,2,w) == 0];
    
    Constraints=[Constraints, 35-lamda1(2,2,w)+mu_ex_gen(2,2,w)>=0];
    Constraints=[Constraints, 25-lamda2(2,2,w)+mu_can_gen(2,2,w)>=0];
 
    Constraints=[Constraints, lamda1(2,2,w)-lamda2(2,2,w)-mu_line(2,2,w)+mu_line_max(2,2,w)-mu_line_min(2,2,w)==0];
    Constraints=[Constraints, -500*mu_line(2,2,w)+mu_del2_max(2,2,w)-mu_del2_min(2,2,w)==0];
    Constraints=[Constraints, 500*mu_line(2,2,w)+ mu_del_ref(2,2,w)==0];
    
    Constraints=[Constraints, mu_ex_gen(2,2,w)>=0];
    Constraints=[Constraints, mu_can_gen(2,2,w)>=0];
    
    Constraints=[Constraints, mu_line_max(2,2,w)>=0];
    Constraints=[Constraints, mu_line_min(2,2,w)>=0];
    Constraints=[Constraints, mu_del1_max(2,2,w)>=0];
    Constraints=[Constraints, mu_del1_min(2,2,w)>=0];
    ...
    Constraints=[Constraints, 35*P_existing_gen(2,2,w)+25*P_can_gen(2,2,w)==Demand(2,2,w)*lamda2(2,2,w)-400*mu_ex_gen(2,2,w)-sum(z_cqo(:,4,w))-...
                              500*(mu_line_max(2,2,w)+mu_line_min(2,2,w)) - pi*(mu_del2_max(2,2,w)+mu_del2_min(2,2,w))];
